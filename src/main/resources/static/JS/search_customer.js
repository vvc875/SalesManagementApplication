...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function handleSearchTypeChange() {
        ...
        // Logic của bạn ở đây đã đúng, giữ nguyên
        if (type) {
            keywordGroup.classList.remove('hidden');
            searchButtonsGroup.classList.remove('hidden');
        } else {
    ...
    function performSearch() {
        ...
        // Tùy thuộc vào giá trị của <select> trong HTML
        // Tôi giả định chúng là 'id', 'name_email', và 'phone'
        if (searchType === 'id') {
            // Khớp với @GetMapping("/{customerId}")
            apiUrl = `http://localhost:8080/customer/${encodeURIComponent(keyword)}`;
        }
        else if (searchType === 'name_email') { // Giả sử value trong HTML là 'name_email'
            // Khớp với @GetMapping("/search")
            apiUrl = `http://localhost:8080/customer/search?keyword=${encodeURIComponent(keyword)}`;
        }
        else if (searchType === 'phone') { // Giả sử value trong HTML là 'phone'
            // Khớp với @GetMapping("/phone")
            // Controller của bạn dùng @RequestParam("keyword"), nên chúng ta gửi "keyword"
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                // Đảm bảo data luôn là một mảng để .forEach() không bị lỗi
                // (Vì tìm theo ID chỉ trả về 1 object, không phải mảng)
                const dataArray = Array.isArray(data) ? data : (data ? [data] : []);
                displayCustomers(dataArray, `Kết quả cho "${keyword}"`);
            })
            .catch(error => {
